...
window_7d as (
    ...
    from {{ ref('fct_contract_prices') }} f
    where f.extraction_ts >= (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '7 days'
    group by 1, 2
),
...
window_30d as (
    select
        ...
        max(f.last_trade_price) - min(f.last_trade_price) as price_range
    from {{ ref('fct_contract_prices') }} f
    where f.extraction_ts >= (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '30 days'
    group by 1, 2
),
...
window_90d as (
    select
        ...
        max(f.last_trade_price) - min(f.last_trade_price) as price_range
    from {{ ref('fct_contract_prices') }} f
    where f.extraction_ts >= (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '90 days'
    group by 1, 2
),
