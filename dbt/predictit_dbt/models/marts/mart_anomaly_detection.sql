...
with stats_30d as (
    ...
    where extraction_ts >= (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '30 days'
    group by 1
),
        ...
        row_number() over (
            ...
            order by abs(
                epoch(extraction_ts) - epoch(
                    (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '24 hours'
                )
            )
        ) as rn
    from {{ ref('fct_contract_prices') }}
    where extraction_ts <= (select max(extraction_ts) from {{ ref('fct_contract_prices') }}) - interval '12 hours'
),
